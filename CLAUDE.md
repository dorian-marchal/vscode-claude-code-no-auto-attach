...
- ⚠️ **Bump `MARKER` (`v39` → next) in [extension.js](extension.js) whenever patch logic changes.** `applyPatch` reverts any older marker before re-applying, so the bump is what makes the rollover seamless.
