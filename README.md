...
- **`extension.js`**
  ...
  - in the `onDidChangeActiveTextEditor` handler, when focus moves to a markdown preview (a `TabInputWebview` whose `viewType` matches `markdown.preview`) and `activeTextEditor` goes undefined, resolves the preview's source `.md` from the tab label's basename — preferring the last active markdown editor, then a unique open markdown document, then a unique `findFiles` match — and pushes it as the current-file context (the same `{filePath,startLine,endLine}` shape upstream produces for an unselected file) instead of clearing it. A second insertion at the top of the handler records the last active markdown editor; both revert by stripping their sentinel blocks.
...
- The markdown-preview resolver fires when focus moves from a text editor to a preview (the common case). Switching directly between two preview tabs doesn't re-fire it, and two identically-named `.md` files that are both closed can't be disambiguated by the tab label — in those cases it falls back to upstream's clear/retain behavior.
